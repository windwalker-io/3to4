...
namespace Windwalker\Legacy\Database\Driver;

use Windwalker\Legacy\Database\Command\AbstractDatabase;
use Windwalker\Legacy\Database\Command\AbstractReader;
use Windwalker\Legacy\Database\Command\AbstractTable;
use Windwalker\Legacy\Database\Command\AbstractTransaction;
use Windwalker\Legacy\Database\Command\AbstractWriter;
use Windwalker\Legacy\Database\Iterator\DataIterator;
use Windwalker\Legacy\Database\Monitor\NullMonitor;
use Windwalker\Legacy\Database\Monitor\QueryMonitorInterface;
...
abstract class AbstractDatabaseDriver implements DatabaseDriverInterface
{
    ...
    public function getQuery($new = false)
    {
        if ($new) {
            ...
            $class = '\\Windwalker\Legacy\\Query\\' . ucfirst($this->name) . '\\' . ucfirst($this->name) . 'Query';
    ...
    public function getTable($name, $new = false)
    {
        if (empty($this->tables[$name]) || $new) {
            $class = sprintf('Windwalker\Legacy\\Database\\Driver\\%s\\%sTable', ucfirst($this->name), ucfirst($this->name));
    ...
    public function getDatabase($name = null, $new = false)
    {
        ...
        if (empty($this->databases[$name]) || $new) {
            $class = sprintf(
                'Windwalker\Legacy\\Database\\Driver\\%s\\%sDatabase',
                ucfirst($this->name),
                ucfirst($this->name)
            );
    ...
    public function getReader($query = null, $new = true)
    {
        ...
        if (!$this->reader || $new) {
            $class = sprintf('Windwalker\Legacy\\Database\\Driver\\%s\\%sReader', ucfirst($this->name), ucfirst($this->name));
    ...
    {
        if (!$this->writer || $new) {
            $class = sprintf('Windwalker\Legacy\\Database\\Driver\\%s\\%sWriter', ucfirst($this->name), ucfirst($this->name));
    ...
    public function getTransaction($nested = true, $new = false)
    {
        if (!$this->transaction || $new) {
            $class = sprintf(
                'Windwalker\Legacy\\Database\\Driver\\%s\\%sTransaction',
                ucfirst($this->name),
                ucfirst($this->name)
            );
    ...
    public function transaction(
        ...
        $trans->transaction($callback, $autoCommit, $enabled);
